...
var resty = function(resourceName) {
    ...
    _self.before = function(path, options) {
        ...
        if (HTTP_METHODS.indexOf(path) !== -1) {
            ...
            httpMethod = path;
            routePath = _getResourcePath(httpMethod);
        }
        else {
     ...
     * @private
     */
    function _getResourcePath(httpMethod) {
        if (!httpMethod || HTTP_METHODS.indexOf(httpMethod) === -1) {
            return _resourcePath;
        }
        else {
            return _getSingularResourcePath();
        }
    }
